...
export type RandomQuizPreviewAccess = {
  has_premium: boolean;
...
export async function getQuizDetail(id: number, lang: Language) {
  return apiRequest<QuizDetail>(`/api/quizzes/${id}?lang=${lang}`, { auth: true });
}
...
export async function getRandomQuizPreview(
  lang: Language,
  payload: {
    total_questions: 30;
    quiz_topic_group_name?: string;
    category_name?: string;
    /** Lọc theo `quiz_categories.id` — dùng khi random theo “Loại chủ đề” (toàn bộ bài cùng category). */
    quiz_category_id?: number;
  }
) {
  return apiRequest<QuizDetail>(`/api/quizzes/random-preview?lang=${lang}`, {
    method: 'POST',
    auth: true,
    body: payload,
  });
...
export async function getRandomQuizPreviewAccess() {
  return apiRequest<RandomQuizPreviewAccess>('/api/quizzes/random-preview-access', {
    auth: true,
  });
}
...
export async function submitAttempt(
  ...
  options?: { elapsed_seconds?: number }
) {
  return apiRequest<SubmitAttemptResult>(`/api/attempts/${attemptId}/submit`, {
    ...
    body: {
      answers,
      ...(options?.elapsed_seconds != null
        ? { elapsed_seconds: Math.floor(Number(options.elapsed_seconds)) }
        : {}),
    },
  });
}
...
export async function getMyQuizAttempts(quizId: number) {
  return apiRequest<QuizAttemptListItem[]>(`/stats/me/quizzes/${quizId}/attempts`, {
    auth: true,
  });
}

export async function getMyAttemptReview(attemptId: number, lang: Language) {
  return apiRequest<AttemptReviewResponse>(
    `/stats/me/attempts/${attemptId}/review?lang=${lang}`,
    {
      auth: true,
    }
  );
